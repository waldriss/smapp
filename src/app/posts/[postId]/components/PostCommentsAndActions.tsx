...
const PostCommentsAndActions = ({
  ...
  const handleDelete = async () => {
    const deletedPost = await deletePost({ postId: post.id,userId:post.posterId.toString() });
    if (deletedPost) {
      router.push("/");
  ...
  const toastContent = (message: string, id: string) => (
    <div className="w-full">
      <h4 className="font-semibold font-sans text-whiteShade text-base ">Notification</h4>
  ...
  return (
    <>
      ...
      <PostActions setlikesNumber={setlikesNumber} postId={post.id} userId={user?.externalId} likers={post.liked_posts} sharers={post.shared_posts} />
      {user?.externalId === post.posterId.toString() && (
        <div className="flex absolute font-sans right-2 top-8 sm:top-7 sm:right-5 gap-x-3 sm:gap-x-4">
          <PencilRuler className="w-6 h-6 sm:h-7 sm:w-7 text-primary cursor-pointer " onClick={()=>router.push(`/postDetails/${post.id}`)}/>
         
        <Trash2 className="w-6 h-6 sm:h-7 sm:w-7 text-primary cursor-pointer " onClick={handleDelete}/>
